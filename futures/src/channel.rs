...
impl Channel {
    /// create a channel
    pub(crate) fn create(channel: mpsc::Sender<Box<dyn Command>>) -> impl Future<Item = Self, Error = io::Error> + Send + 'static {
        ...
        //     })
        // })
        future::err(io::Error::new(io::ErrorKind::Other, "unimplemented!"))
    }
    ...
    pub fn basic_consume(&self, queue: &Queue, consumer_tag: &str, options: BasicConsumeOptions, arguments: FieldTable) -> impl Future<Item = (), Error = io::Error> + Send + 'static {
        // let channel_id = self.id;
        // let transport = self.transport.clone();
